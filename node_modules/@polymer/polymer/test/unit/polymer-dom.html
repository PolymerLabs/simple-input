...
<script type="module">
import '../../polymer.js';
import { dom as $dom } from '../../lib/legacy/polymer.dom.js';
import { Settings as $Settings } from '../../lib/utils/settings.js';
...
suite('legacy api', function() {
  ...
  test('legacy settings', function() {
    assert.equal($Settings.useShadow, !(window.ShadyDOM));
    assert.equal($Settings.useNativeCustomElements, !(window.customElements.polyfillWrapFlushCallback));
    assert.equal($Settings.useNativeCSSProperties, Boolean(!window.ShadyCSS || window.ShadyCSS.nativeCss));
  });
